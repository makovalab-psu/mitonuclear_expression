...
if (length(args)==0) {
  ...
  FILE_IN=args[1]
  data_labels = read.table( "../../ids_aa_ea_GTEx_v8.race", sep="\t" )
  colnames(data_labels) <- c( "FID", "LABEL" )
  FILE_OUT="plot_mds.pdf"
} else if (args[2]=="MERGED"){
  FILE_IN=args[1]
  data_labels = read.table( "../../Merged_GTEx-1KG_v8.ids", sep="\t" )
  colnames(data_labels) <- c( "FID" , "LABEL" )
  FILE_OUT="plot_mds.pdf"
  ...
  FILE_IN=args[1]
  data_labels = read.table( "../../Merged_GTEx-1KG-NATAM_v8.ids", sep="\t" )
  colnames(data_labels) <- c( "FID" , "LABEL" )
  FILE_OUT="plot_mds.pdf"
}
